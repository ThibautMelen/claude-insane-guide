...
**Objectif** : Comprendre les agents et leur invocation

**Exercice** :
1. Demande à Claude :
   ```bash
   "Lance un agent pour explorer le projet et lister tous les fichiers TypeScript"
   ```
2. Observe comment l'agent structure sa recherche
3. Note le temps pris et les résultats

**Résultat attendu** : Liste de fichiers .ts/.tsx avec structure claire

---

### 🟡 Niveau 2 : Utilisation (15 min)

**Objectif** : Créer ton premier sub-agent

**Exercice** :
1. Crée `.claude/plugins/my-agents/index.ts` :
   ```typescript
   export default {
     subAgents: {
       'explainer': {
         ...
         Format: Concept → Analogy → Example`,
...
3. Teste : "Utilise @explainer pour expliquer les closures JavaScript"

**Résultat attendu** : Explication claire avec analogie

---

### 🟠 Niveau 3 : Maîtrise (20 min)

**Objectif** : Workflow multi-agents parallèles

**Exercice** :
1. Crée 3 sub-agents :
   - `security-checker` (Haiku) : Scan basique sécurité
   - `performance-analyzer` (Sonnet) : Analyse performance
   - `accessibility-auditor` (Sonnet) : Audit a11y
2. Demande à Claude :
   ```bash
   "Lance ces 3 agents EN PARALLÈLE pour auditer src/components/Header.tsx"
   ```
3. Compare le temps vs exécution séquentielle

**Résultat attendu** : Rapport complet en 1/3 du temps séquentiel

---

### 🔴 Niveau 4 : Expert (25 min)

**Objectif** : Production vidéo avec 7 agents parallèles

**Exercice** :
1. Crée plugin `.claude/plugins/video-production/`
2. Implémente les 7 agents :
   - transcriber (Haiku)
   - content-analyzer (Sonnet)
   - editor-notes (Sonnet)
   - repetition-detector (Haiku)
   - timeline-analyzer (Opus)
   - broll-suggester (Sonnet)
   - effects-planner (Sonnet)
3. Lance workflow complet sur une vidéo de test

**Résultat attendu** : Production report complet en <3 minutes
