...
**Objectif** : Comprendre workflow EPCT

**Exercice** :
1. Crée projet simple :
   ```bash
   mkdir test-epct && cd test-epct
   echo "<h1>Test</h1>" > index.html
   ```
2. Demande à Claude (sans /epct d'abord) :
   ```
   "Ajoute formulaire contact à index.html"
   ```
3. Observe : Claude code directement (pas de plan)

**Résultat attendu** : Comprendre différence avec/sans EPCT

---

### 🟡 Niveau 2 : Utilisation (15 min)

**Objectif** : Créer et utiliser /epct

**Exercice** :
1. Crée `.claude/commands/epct.md` (voir template ci-dessus)
2. Redémarre Claude Code
3. Teste :
   ```bash
   /epct "Ajoute navbar responsive avec menu burger mobile"
   ```
4. Valide plan proposé
5. Observe implémentation

**Résultat attendu** : Feature créée avec validation préalable

---

### 🟠 Niveau 3 : Maîtrise (20 min)

**Objectif** : Workflow complet avec background tasks
...
1. Projet Vite :
   ...
   ```
2. Lance serveur background :
   ```
   "Lance npm run dev en background"
   ```
3. Pendant que serveur tourne :
   ```bash
   /epct "Créer page About avec sections Mission, Team, Contact"
   ```
4. Observe todo dynamique (Ctrl+T)

**Résultat attendu** : Feature développée avec serveur actif

---

### 🔴 Niveau 4 : Expert (25 min)

**Objectif** : EPCT optimisé avec agents parallèles

**Exercice** :
1. Améliore commande `/epct` pour utiliser agents parallèles en phase Explore :
   ```markdown
   ## Phase 1 : EXPLORE (Avec Agents Parallèles)

   Lance agents EN PARALLÈLE :
   - Agent 1 : Analyser package.json et dépendances
   - Agent 2 : Explorer structure projet
   - Agent 3 : Rechercher documentation (Context7)

   Agrège résultats pour contexte complet.
   ```
2. Teste sur projet complexe

**Résultat attendu** : EPCT 2-3x plus rapide (exploration parallèle)
